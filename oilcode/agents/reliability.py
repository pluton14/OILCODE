...
ПРЕМИСА ПОДТВЕРЖДЕНА ДАННЫМИ (подробности в PROCESS.md, п. 6a).
В апреле 2024 установка стояла (T5 = 13.9 C, расход = 0) — это замена
катализатора. После пуска оба сигнала повели себя ровно так, как предсказывает
технология:
    температура реактора  370-383 C -> 360-368 C  (свежий катализатор активнее)
    перепад давления      2.8-5.4   -> 0.9-2.1    (слой не закоксован)
То есть W10 и T5 действительно отражают старение катализатора и сбрасываются
при его замене.

СЛЕДСТВИЕ, КОТОРОЕ ЛЕГКО УПУСТИТЬ: возраст катализатора — скрытая переменная.
Перцентили, посчитанные по всей истории разом, смешивают состояния до и после
замены. Правильнее калибровать внутри одного цикла катализатора либо добавить
"время с последней замены" признаком.
